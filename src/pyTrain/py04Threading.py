...
def loop():
    print 'thread {0!s} is running...'.format(threading.current_thread().name)
    n = 0 
    while n < 5:
        n = n + 1
        print 'thread {0!s} >>> {1!s}'.format(threading.current_thread().name, n)
        time.sleep(1)
    
    print 'thread {0!s} ended.'.format(threading.current_thread().name)
